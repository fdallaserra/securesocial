...
<<<<<<< HEAD
  ...
  def fillProfile(user: SocialUser): SocialUser = {
    val promise = if (settings.sig) {
      val url : String = "/users/self|access_token=" + user.oAuth2Info.get.accessToken
      val sig : String = hashMac(url, settings.clientSecret)
      WS.url(GetAuthenticatedUser.format(user.oAuth2Info.get.accessToken, sig)).get()
    } else {WS.url(GetAuthenticatedUser.format(user.oAuth2Info.get.accessToken)).get()}
    try {
