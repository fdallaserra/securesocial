...
<<<<<<< HEAD
  private def createSettings(): OAuth2Settings = {
    val result = for {
      ...
      val sig = application.configuration.getBoolean(propertyKey + OAuth2Settings.Sig).getOrElse(false)
      OAuth2Settings(authorizationUrl, accessToken, clientId, clientSecret, scope, authorizationUrlParams, accessTokenUrlParams, sig)
    }
    if ( !result.isDefined ) {
 ...
 */
  protected def hashMac(text : String, secretKey : String) : String = {
    val HASH_ALGORITHM : String  = "HmacSHA256"
    try {
      val sk = new SecretKeySpec(secretKey.getBytes(), HASH_ALGORITHM)
      val mac : Mac = Mac.getInstance(sk.getAlgorithm())
      mac.init(sk)
      val hmac : Array[Byte] = mac.doFinal(text.getBytes())
  ...
  protected def toHexString(bytes : Array[Byte]) : String = {
    var s : String = ""
    for (b <- bytes)
        s += "%02x".format(b)
    return s
...
>>>>>>> db770d3b221162d7e164899d0dd3426d631ebf14
  }
...
<<<<<<< HEAD
                          ...
                          authorizationUrlParams: Map[String, String], accessTokenUrlParams: Map[String, String], sig: Boolean
                           )
=======
  clientSecret: String, scope: Option[String],
